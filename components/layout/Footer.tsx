    ...
    <footer className="border-t border-white/5 bg-[#0D0D0D]">
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12">
        ...
        <div className="lg:col-span-2">
          <div className="mb-6">
            ...
            <span className="text-2xl font-black text-white ml-1">REKLAM</span>
          </div>
          ...
          <div className="flex flex-col gap-3 mb-8">
            <a
              ...
              className="flex items-center gap-2 text-sm text-white/40 hover:text-white transition-colors cursor-none"
            >
              <Phone size={14} className="text-[#FF00C8]" />
              0500 123 45 67
            </a>
            <a
              ...
              className="flex items-center gap-2 text-sm text-white/40 hover:text-white transition-colors cursor-none"
            >
              <Mail size={14} className="text-[#FF00C8]" />
              info@ikarusreklam.com
            </a>
            <span className="flex items-center gap-2 text-sm text-white/40">
              <MapPin size={14} className="text-[#FF00C8]" />
              İstanbul, Türkiye
            </span>
          </div>
          ...
          <div className="flex gap-3">
            {SOCIAL.map(({ label, href, color, svg }) => (
              <a
                ...
                href={href}
                aria-label={label}
                className="w-9 h-9 rounded-full border border-white/8 flex items-center justify-center hover:border-white/20 transition-all cursor-none hover:scale-110"
                style={{ color }}
              >
                {svg}
              </a>
            ...
            <ul className="flex flex-col gap-2.5">
              {links.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="text-sm text-white/45 hover:text-white transition-colors cursor-none"
                  >
                    {link.label}
                  </Link>
                </li>
      ...
      <div className="border-t border-white/5 py-6">
        <div className="max-w-7xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-white/25">© 2026 Ikarus Reklam. Tüm hakları saklıdır.</p>
          <a
