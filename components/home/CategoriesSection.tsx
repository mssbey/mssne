        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {CATEGORIES.map((cat, i) => (
            ...
            >
              <Link
                href={`/${cat.id}`}
                className="group relative flex flex-col p-8 rounded-2xl border border-white/8 bg-[#0D0D0D] hover:border-white/15 transition-all duration-500 hover:-translate-y-1 overflow-hidden cursor-none"
              >
                {/* Glow */}
                <div
                ...
                <p className="text-sm text-white/40 mb-4 flex-1">{cat.desc}</p>
                <div className="flex items-center justify-between">
                  ...
                  <ArrowRight
                    size={16}
                    className="text-white/20 transition-all group-hover:text-white/70 group-hover:translate-x-1"
                  />
                </div>
              </Link>
            </motion.div>
