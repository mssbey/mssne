        ...
        <div className="flex justify-center mb-12">
          <div className="relative w-full max-w-sm">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 rounded-full bg-black/5 border border-black/10 text-sm text-black placeholder:text-black/25 outline-none focus:border-[#D9668A]/60 transition-colors"
            />
          </div>
        </div>
            ...
            <AnimatePresence>
              {filtered.map((no) => (
                <motion.div
                  ...
                  className="group relative rounded-2xl border border-black/8 overflow-hidden cursor-none hover:border-[#D9668A]/40 transition-all bg-[#FFFFFF]"
                >
                  ...
                  <div className="flex items-center justify-between px-4 py-3">
                    ...
                    <span className="text-[10px] uppercase tracking-[2px] text-black/30">
                      Davetiye
                    </span>
                  </div>
                </motion.div>
        ...
        {selected && (
          ...
          >
            <motion.div
              ...
              className="relative max-w-3xl w-full rounded-3xl border border-black/15 overflow-hidden bg-[#FFFFFF]"
            >
              <button
                onClick={() => setSelected(null)}
                className="absolute top-4 right-4 z-10 w-9 h-9 rounded-full bg-black/60 flex items-center justify-center text-white/70 hover:text-white cursor-none"
              >
                <X size={18} />
              </button>
              ...
              {selectedIndex > 0 && (
                <button
                  onClick={goPrev}
                  className="absolute left-3 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-black/60 flex items-center justify-center text-white/70 hover:text-white cursor-none"
                >
                  <ChevronLeft size={20} />
                </button>
              )}
              {selectedIndex >= 0 && selectedIndex < filtered.length - 1 && (
                <button
                  onClick={goNext}
                  className="absolute right-3 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-black/60 flex items-center justify-center text-white/70 hover:text-white cursor-none"
                >
