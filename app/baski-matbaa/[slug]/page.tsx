...
import { BASKI_MATBAA, getBaskiMatbaaBySlug, getOtherBaskiMatbaa } from "@/lib/baski-matbaa";
...
export async function generateStaticParams() {
  return BASKI_MATBAA.map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const service = getBaskiMatbaaBySlug(slug);
  if (!service) return {};
...
export default async function BaskiMatbaaDetailPage({ params }: Props) {
  const { slug } = await params;
  const service = getBaskiMatbaaBySlug(slug);
  if (!service) notFound();
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Baskı ve Matbaa", item: `${SITE_URL}/baski-matbaa` },
  ...
  return (
    <>
      ...
      <RelatedServices
        services={others}
        eyebrow="Diğer Baskı Hizmetlerimiz"
        heading="Baskı ve Matbaa Çözümlerimizin Tamamını Keşfedin"
      />
    </>
  );
}
